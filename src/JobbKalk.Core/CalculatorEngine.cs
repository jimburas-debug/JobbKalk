...
public static class CalculatorEngine
{
    ...
    private static CalculationResult OhmsLaw(IReadOnlyDictionary<string, double?> v)
    {
        ...
        return R(new ResultItem("Spenning", u!.Value, "V"), new ResultItem("Strøm", i!.Value, "A"),
            new ResultItem("Motstand", r!.Value, "Ω"), new ResultItem("Effekt", u.Value * i.Value, "W"));
    }
    ...
    private static CalculationResult OnePhase(IReadOnlyDictionary<string, double?> v)
    {
        ...
        return R(new ResultItem("Beregnet strøm", p * 1000 / (u * pf * eta), "A"));
    }
    ...
    private static CalculationResult ThreePhase(IReadOnlyDictionary<string, double?> v)
    {
        var p = Get(v, "p"); var u = Get(v, "u", true); var pf = Factor(Get(v, "pf"), "Effektfaktor");
        var eta = PercentFactor(Get(v, "eta"), "Virkningsgrad");
        return R(new ResultItem("Beregnet linjestrøm", p * 1000 / (Math.Sqrt(3) * u * pf * eta), "A"));
    }

    private static CalculationResult EnergyCost(IReadOnlyDictionary<string, double?> v)
    {
        var energy = Get(v, "p") * Get(v, "hours");
        return R(new ResultItem("Energibruk", energy, "kWh"), new ResultItem("Kostnad", energy * Get(v, "price"), "kr"));
    }
    ...
    private static CalculationResult VoltageDrop(IReadOnlyDictionary<string, double?> v, bool threePhase)
    {
        ...
        return new([new ResultItem("Spenningsfall", drop, "V"), new ResultItem("Spenningsfall", drop / voltage * 100, "%")],
    ...
    private static CalculationResult AreaWaste(IReadOnlyDictionary<string, double?> v)
    {
        ...
        return R(new ResultItem("Netto areal", area, "m²"), new ResultItem("Materialbehov", area * (1 + Get(v, "waste") / 100), "m²"));
    }
    ...
    private static CalculationResult Volume(IReadOnlyDictionary<string, double?> v)
    {
        ...
        return R(new ResultItem("Volum", volume, "m³"), new ResultItem("Volum", volume * 1000, "liter", 1));
    }
    ...
    private static CalculationResult Concrete(IReadOnlyDictionary<string, double?> v)
    {
        ...
        return R(new ResultItem("Netto volum", net, "m³", 3), new ResultItem("Bestillingsmengde", net * (1 + Get(v, "waste") / 100), "m³", 3));
    }
    ...
    private static CalculationResult Paint(IReadOnlyDictionary<string, double?> v)
    {
        ...
        return R(new ResultItem("Anslått maling", liters, "liter"));
    }
    ...
    private static CalculationResult Flooring(IReadOnlyDictionary<string, double?> v)
    {
        ...
        return R(new ResultItem("Materialbehov", material, "m²"), new ResultItem("Hele pakker", Math.Ceiling(material / pack - 1e-10), "stk", 0));
    }

    private static CalculationResult RoofPitch(IReadOnlyDictionary<string, double?> v)
    {
        var rise = Get(v, "rise"); var run = Get(v, "run", true);
        return R(new ResultItem("Takvinkel", Math.Atan(rise / run) * 180 / Math.PI, "°", 1),
            new ResultItem("Skrålengde", Math.Sqrt(rise * rise + run * run), "m"));
    }

    private static CalculationResult FuelConsumption(IReadOnlyDictionary<string, double?> v) =>
        R(new ResultItem("Forbruk", Get(v, "liters") / Get(v, "distance", true) * 100, "l/100 km"));
    ...
    private static CalculationResult TripCost(IReadOnlyDictionary<string, double?> v)
    {
        ...
        return R(new ResultItem("Drivstoff", liters, "liter"), new ResultItem("Kostnad", liters * Get(v, "price"), "kr"));
    }

    private static CalculationResult CostPerKm(IReadOnlyDictionary<string, double?> v) =>
        R(new ResultItem("Kostnad", Get(v, "cost") / Get(v, "distance", true), "kr/km"));
    ...
    private static CalculationResult Payload(IReadOnlyDictionary<string, double?> v)
    {
        ...
        return R(new ResultItem("Maksimal nyttelast", max, "kg", 0), new ResultItem("Gjenstående nyttelast", remaining, "kg", 0));
    }
    ...
    private static CalculationResult TravelTime(IReadOnlyDictionary<string, double?> v)
    {
        ...
        return R(new ResultItem("Timer", whole, "t", 0), new ResultItem("Minutter", Math.Round((hours - whole) * 60), "min", 0));
    }

    private static CalculationResult Percentage(IReadOnlyDictionary<string, double?> v) =>
        R(new ResultItem("Resultat", Get(v, "value") * Get(v, "percent") / 100, ""));

    private static CalculationResult PercentChange(IReadOnlyDictionary<string, double?> v) =>
        R(new ResultItem("Endring", (Get(v, "new") - Get(v, "old", true)) / Get(v, "old", true) * 100, "%"));
    ...
    {
        var m = Get(v, "meters");
        return R(new ResultItem("Millimeter", m * 1000, "mm"), new ResultItem("Centimeter", m * 100, "cm"), new ResultItem("Kilometer", m / 1000, "km", 4));
    }

    private static CalculationResult Temperature(IReadOnlyDictionary<string, double?> v)
    {
        var c = Get(v, "c");
        return R(new ResultItem("Fahrenheit", c * 9 / 5 + 32, "°F", 1), new ResultItem("Kelvin", c + 273.15, "K", 2));
    }
    ...
    private static CalculationResult MarkupMargin(IReadOnlyDictionary<string, double?> v)
    {
        ...
        return R(new ResultItem("Salgspris", sale, "kr"), new ResultItem("Bruttofortjeneste", sale - cost, "kr"),
            new ResultItem("Bruttomargin", sale == 0 ? 0 : (sale - cost) / sale * 100, "%"));
    }
